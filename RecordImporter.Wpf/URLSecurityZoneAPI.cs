...
namespace RecordImporter.Wpf
{
    ...
    public class URLSecurityZoneAPI
    {
        ...
        {
            return (SafeNativeMethods.CoInternetSetFeatureEnabled((int)feature, (int)target, enabled) == 0);
        }
        ...
        /// <param name="feature">The feature from <c>InternetFeaturelist</c></param>
        /// <param name="target">The target from <c>SetFeatureOn</c></param>
        /// <returns><c>true</c> if the internet feature is enabled; otherwise, <c>false</c>.</returns>
        public static bool IsInternetSetFeatureEnabled(InternetFeaturelist feature, SetFeatureOn target)
        {
            return (SafeNativeMethods.CoInternetIsFeatureEnabled((int)feature, (int)target) == 0);
        }
    }
