...
namespace PortableDevices
{
    public class PortableDevice
    {
        ...
        public PortableDevice(string deviceId) : this(deviceId, null, null, null)
        { }

        public PortableDevice(string deviceId, string friendlyName, string description, string manufacturer)
        {
        ...
        public PortableDeviceFolder Root { get; set; }
        ...
        public void GetContents(string name)
        {
            Root = new PortableDeviceFolder("DEVICE", name);

            IPortableDeviceContent content;
            PortableDeviceClass.Content(out content);

            EnumerateContents2(ref content, Root);
        }

        public void GetContents(ref PortableDeviceFolder folder)
        {

            IPortableDeviceContent content;
            PortableDeviceClass.Content(out content);

            EnumerateContents2(ref content, folder);
        }

        private static void EnumerateContents2(ref IPortableDeviceContent content,
            PortableDeviceFolder parent)
        {
            ...
            do
            {
                ...
                //if (currentObject.Name.ToLower().Replace(" ", "") != "sdcard" && currentObject.Name.ToLower().Replace(" ", "") != "card") continue;

                parent.Files.Add(currentObject);

                //if (currentObject is PortableDeviceFolder)
                //{
                //    EnumerateContents3(ref content, (PortableDeviceFolder)currentObject);
                //}
            } while (fetched > 0);
        }

        private static void EnumerateContents3(ref IPortableDeviceContent content,
            PortableDeviceFolder parent)
        {
            // Get the properties of the object
            IPortableDeviceProperties properties;
        ...
        public void DownloadFile(PortableDeviceFile file, string saveToPath)
        {
            ...
            unsafe
            {
                ...
                targetStream.SetLength(bytesReadMax); 
                targetStream.Close();
            ...
            string objectId)
        {
            ...
            {
                return new PortableDeviceFile(objectId, "-Error Occurred");
            }
