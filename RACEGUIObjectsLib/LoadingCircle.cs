...
namespace RaceGUIObjectsLib
{
    public partial class LoadingCircle : Control
    {
        ...
        //setup different style wait circles - well they look a bit like them anyway!
        ...
        private const int RaceInnerCircleRadius = 8;
        private const int RaceOuterCircleRadius = 9;
        private const int RaceNumberOfSpoke = 22;
        private const int RaceSpokeThickness = 4;
        private static int RaceSpeedUp = 4;
            ...
            set
            {
                ...
                switch (m_StylePreset)
                {
                    ...
                    case StylePresets.IE7:
                        ...
                        break;
                    case StylePresets.Race:
                        SetCircleAppearance(RaceNumberOfSpoke,
                            RaceSpokeThickness, RaceInnerCircleRadius,
                            RaceOuterCircleRadius);
                        this.Color = System.Drawing.Color.CornflowerBlue;
                        break;
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            if(m_NumberOfSpoke > 0){
                ...
                if(Active) { // only if active...otherwise it will run in designer ;)
                    switch(m_StylePreset) {
                        case StylePresets.Race: // for Race style
                            if(RaceSpeedUp == 0) {
                                if(m_NumberOfSpoke > 2) {
                                    m_NumberOfSpoke--;
                                }
                                else {
                                    m_NumberOfSpoke = RaceNumberOfSpoke;
                                }
                                GenerateColorsPallet();
                                GetSpokesAngles();
                            }
                            // speeding it up when spokes getting less
                            if(RaceSpeedUp == 0) {
                                if(m_NumberOfSpoke > 18) {
                                    RaceSpeedUp = 4;
                                }
                                else {
                                    if(m_NumberOfSpoke > 12) {
                                        RaceSpeedUp = 3;
                                    }
                                    else {
                                        if(m_NumberOfSpoke > 5) {
                                            RaceSpeedUp = 2;
                                        }
                                        else {
                                            RaceSpeedUp = 1;
